...
export function DonutChart({
  ...
  const segments = data.map((d, i) => {
    ...
    angle = end;
    return {
      path: arcPath(cx, cy, r, start, Math.max(start + 0.5, end)),
    ...
    <View style={styles.donutRow}>
      <View style={{ width: size, height: size }}>
        <Svg width={size} height={size}>
          ...
          {segments.map((s, i) => (
            <Path key={i} d={s.path} stroke={s.color} strokeWidth={stroke} fill="none" strokeLinecap="butt" />
          ))}
        </Svg>
        ...
        {data.map((d, i) => {
          ...
          return (
            <View key={d.label} style={styles.legendRow} testID={`legend-${d.label}`}>
              <View style={[styles.swatch, { backgroundColor: chartPalette[i % chartPalette.length] }]} />
        ...
        return (
          <View key={d.label} testID={`bar-${d.label}`}>
            <View style={styles.barHeader}>
              ...
              <Text style={[styles.barCount, { color: colors.onSurface, fontSize: fontSize.sm }]}>{d.count}</Text>
            </View>
...
/** A single labelled progress bar (tagged / taggable, etc.). */
export function ProgressBar({
  ...
  return (
    <View>
      {(label || right) && (
        <View style={styles.barHeader}>
          <Text style={{ color: colors.onSurfaceSecondary, fontSize: fontSize.sm }}>{label}</Text>
          <Text style={{ color: colors.onSurface, fontSize: fontSize.sm, fontWeight: "700" }}>{right ?? `${pct}%`}</Text>
        </View>
      )}
      <View style={[styles.barTrack, { backgroundColor: colors.surfaceTertiary, height: 10 }]}>
        <View style={{ width: `${pct}%`, height: "100%", backgroundColor: colors.brand, borderRadius: 2 }} />
      </View>
    </View>
  );
}
