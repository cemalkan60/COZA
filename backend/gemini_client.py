...
_BRAND_CITY_PROMPT = (
    "You are a fashion industry expert. Every one of these brand names "
    "comes from a runway-photography archive (fashion-press.net / "
    "firstview.com), so each one really does show collections at a major "
    "fashion week somewhere — your job is to say where. For each brand "
    "name below, answer which of the 4 fashion-month cities (New York, "
    "London, Milan, or Paris) that house shows its REGULAR ready-to-wear/"
    "haute couture runway collections in — not a one-off special/resort/"
    "destination show. Give your single best answer for every brand, even "
    "if you're not 100% certain — pick the city it's most associated with "
    "rather than abstaining. Only answer null if the name is genuinely not "
    "a real fashion house at all (garbled text, not a brand).\n\n"
...
_SHOW_CITY_PROMPT = (
    ...
    "from a runway-photography archive — it really was held somewhere, "
    "your job is to say where. Unlike a brand's regular ready-to-wear/"
    "couture show, these are frequently staged as special one-off events "
    "in a notable location — often named or hinted at right in the show's "
    "own title/theme (e.g. a \"Cruise 2024\" show held in Seoul, a "
    "\"Croisière\" show held in Marrakech). Give your single best answer "
    "for every entry, even if you're not 100% certain:\n"
    "1. If you know (or can make a well-informed best guess at) the "
    "specific city THAT show was held in, answer that.\n"
    "2. Otherwise, fall back to the city where that brand's REGULAR "
    "fashion-week runway shows normally take place.\n"
    "Only answer null if the brand name itself is genuinely not a real "
    "fashion house at all (garbled text, not a brand).\n\n"
